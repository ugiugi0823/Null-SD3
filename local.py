...
# tokenizer_CLIP_L_14 = AutoTokenizer.from_pretrained("openai/clip-vit-large-patch14")
...
class AttentionReplace(AttentionControlEdit):
    ...
    def replace_cross_attention(self, attn_base, att_replace):
        # expanded_mapper = self.mapper.expand(1, 154, 154)
        # attn_base_chunked = torch.chunk(attn_base, chunks=2, dim=-1)[0]
...
def text2image_ldm_stable(
    ...
    model.default_sample_size = model.transformer.config.sample_size
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
        ...
        latents = ptp_utils.diffusion_step(model, controller, latents, context, context_p, i, t, guidance_scale, low_resource=False)
